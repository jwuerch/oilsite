...
if(!class_exists('Facebook_Like_Widget')){

	class Facebook_Like_Widget extends WP_Widget {

		function __construct()
		{
			$widget_ops = array('classname' => 'facebook_like', 'description' => __('Add Facebook Like Box.', 'schema' ));

			$control_ops = array('id_base' => 'facebook-like-widget');

			parent::__construct('facebook-like-widget', sprintf( __('%sFB Like Box', 'schema' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ), $widget_ops, $control_ops);
		}
		...
		{
			$defaults = array('title' => __('Find us on Facebook', 'schema' ), 'page_url' => '', 'width' => '292', 'show_faces' => 'on', 'show_stream' => false, 'show_header' => 'on');
			...
			<p>
				<label for="<?php echo $this->get_field_id('page_url'); ?>"><?php _e('Facebook Page URL', 'schema' ); ?>:</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('width'); ?>"><?php _e('Width', 'schema' ); ?>:</label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_faces'); ?>"><?php _e('Show faces', 'schema' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_stream'); ?>"><?php _e('Show stream', 'schema' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_header'); ?>"><?php _e('Show facebook header', 'schema' ); ?></label>
			</p>
		<?php
		}
	}
